...
- The repository-level Cloud Build executor at `../cloudbuild.playwright-e2e.json` fails closed unless `COMMIT_SHA^{commit}` equals `HEAD^{commit}` and accepts only `smoke` (safe default) or `full`. Standard fixture specs under `tests/e2e/**/*.spec.ts` join smoke automatically except `maintenance.spec.ts` and `production/**`; the exact maintenance filename joins full through `playwright.maintenance.config.ts`; production specs remain isolated under `playwright.production.config.ts`. Same-repository PRs to `development` derive smoke, and the internal `development -> staging` route derives full. PR #270 proved manual same-SHA full-suite parity at `317483acb566fbeb6077a0c10a8c70f721e1c2ca`. PR #273 proved automatic staging OIDC full dispatch for exact head SHA `16fab46f3781088ea4b310d6bb4e265c170372a9`: GitHub run `34409554758` and Cloud Build `dcf5a6e9-b9b1-430d-a41e-cf98a9e84ade` succeeded with matching source revision and `COMMIT_SHA`, `_PLAYWRIGHT_SUITE=full`, and parallel GitHub full-suite and governance checks passing before merge commit `983c933f9b0a77ccdbb8472d0bcecd3bc425eb8e`. Maintainer acceptance removed the redundant GitHub-hosted `chromium-smoke` and `chromium-full` jobs and their `pull_request` trigger; `production-public-smoke` remains unchanged and isolated. Keep OIDC keyless, dependencies frozen, images digest-pinned, and all fixtures synthetic and isolated from GCS, IAM, secrets, Cloud SQL, Cloud Run, staging, and production. Production-smoke migration, authenticated real-stack E2E completion, diagnostic artifacts, and disabled-trigger cleanup remain pending under #261; deployments and further GCP/IAM changes remain outside this cutover.
- Fixture and production discovery remain isolated from `playwright.real-auth.config.ts`, which selects only `tests/e2e-real-auth/real-auth.spec.ts` with Chromium, one worker, no retries, and serial scenarios. The independent readiness step remains unauthenticated and write-free; the following real-auth step owns an isolated PostgreSQL/Strapi/Next.js stack and synthetic credentials/data. PR #281 proved corrected readiness teardown at `b35f622eb74d24a847745eade98af6d5af594442` through Cloud Build `7f7cc50d-4bb1-4511-8297-0d8ab3e68f39`. The new real-auth capability has static verification only and still needs authorized real Docker/Cloud Build proof. Keep OIDC keyless, dependencies frozen, images digest-pinned, JWT server-only, and all resources isolated from GCS, IAM, secrets, Cloud SQL, Cloud Run, staging, and production. Production-smoke migration and disabled-trigger cleanup remain pending under #261.
