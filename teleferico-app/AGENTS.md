...
- The repository-level Cloud Build executor at `../cloudbuild.playwright-e2e.json` fails closed unless `COMMIT_SHA^{commit}` equals `HEAD^{commit}` and accepts only `smoke` (safe default) or `full`. Standard fixture specs under `tests/e2e/**/*.spec.ts` join smoke automatically except `maintenance.spec.ts` and `production/**`; the exact maintenance filename joins full through `playwright.maintenance.config.ts`; production specs remain isolated under `playwright.production.config.ts`. Same-repository PRs to `development` derive smoke, and the implemented internal `development -> staging` route derives full. GitHub parity checks out the same exact head SHA. Because `pull_request_target` uses workflow code from default `main`, PR #269 checks prove only development smoke. Bootstrap the first promotion with GitHub `chromium-full` and an explicitly approved manual exact-SHA `_PLAYWRIGHT_SUITE=full` run; this does not prove automatic OIDC staging dispatch. Only a later promotion can prove that path after the workflow reaches `main` and the WIF condition is separately approved and expanded. Keep OIDC keyless, dependencies frozen, images digest-pinned, and all fixtures synthetic and isolated from GCS, IAM, secrets, Cloud SQL, Cloud Run, staging, and production. Production smoke, authenticated E2E, diagnostic artifacts, redundant-job removal, Vitest migration, deployments, and disabled-trigger deletion remain out of scope.
