...
- The repository-level Cloud Build baseline at `../cloudbuild.playwright-e2e.json` preserves the fixture-backed smoke suite and fails closed when `COMMIT_SHA^{commit}` cannot be verified against `HEAD^{commit}`. Every Node step enables Corepack before package work because Cloud Build containers are isolated. Its fourth fixture pilot passed for `498004d7f065e6b0a43bff42dd95c672efc2b707`; the first real-stack readiness pilot (`93242141-7e58-4c2d-b6f6-7673cb5eb33b`) failed when production mode selected the GCS upload provider without a bucket. The corrected isolated runner uses `NODE_ENV=test` and Strapi's default local upload storage, leaving staging and production GCS configuration untouched. Rerun `0e61da83-7490-4208-b242-ee04a95ba9b5` reached PostgreSQL and Strapi `/admin/init` readiness, but Next.js failed before readiness because `pnpm run dev -- --hostname 0.0.0.0 --port 3000` passed a doubled delimiter that made `--hostname` a positional project directory. Build `323d2e61-5f11-45c3-8947-240ee3023d6a` confirmed that PostgreSQL and Strapi succeeded and the corrected Next CLI started, then failed config validation because the runner omitted `BUILD_STRAPI_BUCKET_HOSTNAME`. The runner now supplies `BUILD_STRAPI_BUCKET_HOSTNAME=127.0.0.1` and `BUILD_STRAPI_BUCKET_PATHNAME=/uploads/**` as synthetic local image inputs; a rerun is pending, so the full real stack is not yet proven. It must stay unauthenticated, write-free, synthetic, and isolated on the `cloudbuild` Docker network. It must not introduce Cloud SQL, Cloud Run, staging, or production dependencies.
