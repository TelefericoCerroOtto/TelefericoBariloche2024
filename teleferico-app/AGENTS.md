...
- The repository-level Cloud Build executor at `../cloudbuild.playwright-e2e.json` fails closed unless `COMMIT_SHA^{commit}` equals `HEAD^{commit}` and accepts only `smoke` (safe default) or `full`. Standard fixture specs under `tests/e2e/**/*.spec.ts` join smoke automatically except `maintenance.spec.ts` and `production/**`; the exact maintenance filename joins full through `playwright.maintenance.config.ts`; production specs remain isolated under `playwright.production.config.ts`. Same-repository PRs to `development` derive smoke, and the internal `development -> staging` route derives full. PR #270 proved manual same-SHA full-suite parity at `317483acb566fbeb6077a0c10a8c70f721e1c2ca` through GitHub run `34399083297` and Cloud Build `b2639432-a76e-4c65-b2f3-5c6fd779afc6`. The workflow is now on default `main`, and the WIF prerequisite now permits the staging route; automatic staging OIDC full dispatch is configured but still needs a later genuine eligible PR for proof. Keep the GitHub-hosted `chromium-smoke`, temporary `chromium-full`, and `production-public-smoke` jobs during parity. Keep OIDC keyless, dependencies frozen, images digest-pinned, and all fixtures synthetic and isolated from GCS, IAM, secrets, Cloud SQL, Cloud Run, staging, and production. Production-smoke migration and final redundant-job removal remain pending under #261; authenticated E2E, diagnostic artifacts, Vitest migration, deployments, and disabled-trigger deletion remain outside this proof.
