...
- The current automated-test inventory includes Vitest unit, component, guard, and Route Handler coverage plus Playwright E2E coverage. Vitest discovers only `src/**/*.{test,spec}.{ts,tsx}`; tests outside `src` and JavaScript or JSX test files do not join automatically.
- Behavior-changing code changes MUST run focused tests for the changed behavior. New features MUST include representative tests; forms and security changes MUST keep relevant Vitest coverage passing.
- Changes to `src/lib/http/guards`, `src/lib/services/form-protection.ts`, or the form API routes MUST add or update relevant Vitest coverage and run those focused tests successfully; this requirement is not waived by a later broad-suite failure. Documentation-only changes do not require tests.
- Do not run the full app Vitest suite by default during direct implementation. Until required PR CI adopts app Vitest, `/implementation-pr` runs it once after governance observation when the complete candidate inventory contains app code, config, tests, assets, or unknown file types. App Markdown-only docs (including `README.md`), docs outside the app, and CMS-only changes skip the suite. This post-creation check is advisory to publication and never creates, delays, or blocks the PR. On failure, the helper may report only bounded validated repo-relative failed test-file paths and counts from a private temporary JSON report; it withholds raw output, test names, failure messages, and stacks. Missing or unusable reports leave diagnostics unavailable without changing the test result. A temporary-directory cleanup failure must preserve the completed Vitest status and exit code, exposing only a fixed cleanup warning. Retire the local run rule and helper in the same policy change that makes the suite a required CI check.
...
- The repository-level Cloud Build executor at `../cloudbuild.playwright-e2e.json` fails closed unless `COMMIT_SHA^{commit}` equals `HEAD^{commit}` and accepts only `smoke` (safe default) or `full`. Standard fixture specs under `tests/e2e/**/*.spec.ts` join smoke automatically except `maintenance.spec.ts` and `production/**`; the exact maintenance filename joins full through `playwright.maintenance.config.ts`; production specs remain isolated under `playwright.production.config.ts`. Every accepted legacy profile then runs independent real-stack readiness followed by blocking real-auth acceptance. Same-repository PRs to `development` derive smoke, and the internal `development -> staging` route derives full. Keep OIDC keyless, dependencies frozen, images digest-pinned, and all fixtures synthetic and isolated from GCS, IAM, secrets, Cloud SQL, Cloud Run, staging, and production.
- Fixture and production discovery remain isolated from `playwright.real-auth.config.ts`, which selects only `tests/e2e-real-auth/real-auth.spec.ts` with Chromium, one worker, no retries, and three serial scenarios. The inner real-auth lifecycle records bounded validated evidence only after Playwright exits successfully, its structured report proves exactly three discovered non-skipped passing scenarios, synthetic state is verified and restored, and services are cleaned up. The outer finalizer may emit `TB122 real-auth acceptance=scenarios:3/3` only after validating that evidence, repeating final PostgreSQL readiness, removing owned containers, proving their absence, removing diagnostics, and selecting exit `0`. Merge evidence requires that marker and terminal Cloud Build success for the exact PR SHA; local checks do not prove it. The step owns an isolated PostgreSQL/Strapi/Next.js stack and synthetic credentials/data, preserves bounded cleanup, and sends no secrets to Cloud Build configuration. Production smoke, deployments, persistent environments, role hardening, and disabled-trigger cleanup remain outside this slice under #261.
