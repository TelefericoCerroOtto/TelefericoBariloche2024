...
- The repository-level Cloud Build baseline at `../cloudbuild.playwright-e2e.json` fails closed unless `COMMIT_SHA^{commit}` equals `HEAD^{commit}` and runs five isolated fixture/readiness steps. PR #268 proved the automatic GitHub OIDC dispatcher and same-SHA GitHub Chromium parity for `8a0185fa70ee0dedc53573f0f6dafbffd9a4199c`: dispatcher build `b29e46c1-10a4-4d28-bf2b-21b7d985b22d` and final legacy `/gcbrun` build `3ce4566d-030c-4dcf-b533-f9c97dc71b7c` both passed. The legacy `playwright-e2e-pr` trigger is disabled; future PR updates use the automatic dispatcher plus GitHub parity smoke, while the manual exact-SHA trigger remains the fallback executor. Retain the disabled trigger only until a later dispatcher-only SHA proves its check does not return. This covers development fixture smoke only, not staging, production-smoke migration, authenticated E2E, diagnostic artifacts, or redundant-job removal. The dispatcher paths still exclude their own workflow and static test; issue #261 tracks that gap. The runner must remain unauthenticated, write-free, synthetic, and isolated from GCS, IAM, secrets, Cloud SQL, Cloud Run, staging, and production.
