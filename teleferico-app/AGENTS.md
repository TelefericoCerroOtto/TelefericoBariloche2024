...
- The repository-level Cloud Build executor at `../cloudbuild.playwright-e2e.json` fails closed unless `COMMIT_SHA^{commit}` equals `HEAD^{commit}` and accepts only `smoke` (safe default) or `full`. Standard fixture specs under `tests/e2e/**/*.spec.ts` join smoke automatically except `maintenance.spec.ts` and `production/**`; the exact maintenance filename joins full through `playwright.maintenance.config.ts`; production specs remain isolated under `playwright.production.config.ts`. Same-repository PRs to `development` derive smoke, and only internal `development -> staging` promotions derive full. GitHub parity checks out the same exact head SHA. PR #268 and dispatcher-only run `34381586085` prove only development smoke; full remains unproven until the WIF base condition is expanded by an approved operator and a promotion passes both executors. Keep OIDC keyless, dependencies frozen, images digest-pinned, and all fixtures synthetic and isolated from GCS, IAM, secrets, Cloud SQL, Cloud Run, staging, and production. Production smoke, authenticated E2E, diagnostic artifacts, redundant-job removal, and disabled-trigger deletion remain out of scope.
