...
- Ordinary finalization uses one cheap local branch/staging/whitespace check and existing hooks; do not invoke `commit-guard` as a repeated or universal gate. For behavior-changing TypeScript in `teleferico-app`, require focused tests and exactly one `pnpm run typecheck`, preferably before the publication candidate is frozen. Record the exact command, result, and candidate identity in writer evidence. Reuse that result later only when the same candidate identity and worktree/input state are proven; otherwise run typecheck once against the publication candidate before commit. A candidate-caused failure blocks completion; unrelated or unknown failures remain partial and must be reported, never called passing. Passive documentation-only changes need no semantic typecheck. Do not create a cross-session cache, receipt, stamp, or new helper. Preserve the candidate/staged-content before-and-after proof at commit time.
...
4. Immediately before commit, run one cheap local staging/whitespace check (`git diff --cached --check` and `git diff --check`), then prove the candidate path set and bytes match the captured snapshot and revalidate only branch, `HEAD`, and the invocation authorization binding needed for this operation. When changes exist, invoke `commit-planner` auto mode for only the captured snapshot. Require a clean tree and verify the commit matches the captured candidate before continuing.
