...
- Inspect native SDD state only when the caller's explicit `route` is a formal SDD lifecycle operation or boundary. For `route=direct`, follow the change-local route and do not perform generic OpenSpec/SDD discovery. Reuse any applicable SDD observations already collected for this snapshot.
...
- Resolve the repository root, selected remote, branch, full `HEAD`, upstream, porcelain worktree state, candidate paths, and base relation. Return the complete sorted candidate path inventory without truncation. Classify every candidate path against credential/sensitive path semantics and the caller's captured candidate scope; use path/metadata only and never read suspected credential contents. A tracked regular-file template whose basename is exactly `.env.example`, `.env.sample`, or `.env.template` is not sensitive solely because of that basename's `.env` prefix, provided no ancestor path segment is sensitive. This narrow path exception does not establish that file contents are safe: do not inspect or attest contents, and keep other environment-file names, secret-like basenames, sensitive ancestors, and any separately identified content concern blocked. Bound classification examples to 5.
...
- Only for an explicitly selected formal SDD route, inspect `openspec/config.yaml` and the selected active change's `tasks.md`, `apply-progress.md`, and `verify-report.md` when present. Report recorded phase, current work unit, completion/evidence facts, remaining work, delivery strategy, review budget, and predecessor identity. Do not reinterpret test evidence or treat strict TDD as a delivery strategy.
...
- Revalidate the mixed-scope override binding before returning: the complete candidate snapshot, branch and `HEAD`, selected `origin`, target/base plan, destination, and current authenticated Git/GitHub session authorization must be unchanged. A changed or unverifiable binding is a blocker. One authorization may cover the full exact non-sensitive unrelated inventory; never require separate approval for each path. The mapper reports facts and evidence only; `implementation-pr` owns activation, disclosure handoff, and every mutation decision.
- Return a self-consistent complete inventory in the snapshot. If local evidence cannot establish exact equality between `path_count` and `paths`, or path classifications conflict with the caller's candidate binding, report the whole snapshot as incomplete/ambiguous; never return a count-only correction. The coordinator may discard that snapshot and request at most one bounded complete replacement. Any candidate or authorization-binding change requires fresh authorization before publication can continue.
...
Keep `evidence` to at most 12 entries. Values must be scalar or short bounded arrays, except that `mixed_scope_exceptional_paths` may carry the complete exact sorted non-sensitive unrelated path inventory because it is the audit record handed to `implementation-pr`; it must never be truncated. Emit that path inventory whenever complete and non-sensitive, including when the caller did not supply an override. Record separate evidence facts for `mixed_scope_override`, `mixed_scope_override_reason`, and `mixed_scope_exceptional_paths` when applicable. The recommendation is informational only; consumers own eligibility, policy, authorization, disclosure verification, and every mutation. `observed_at` records observation time, not durable truth; consumers must revalidate immediately before mutation.
