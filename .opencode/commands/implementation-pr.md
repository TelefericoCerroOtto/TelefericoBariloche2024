...
The slash command accepts bare `/implementation-pr` as the strict-scope default, or exactly `/implementation-pr --allow-mixed-scope` to select known non-sensitive paths outside the addressed item. No justification is required; unknown or extra arguments and alternate spellings are rejected. An explicit natural-language scope selection is also valid. Scope selection does not authorize publication: the workflow separately requires current explicit authorization to commit the candidate, non-force-push `HEAD` to the named remote, and create one implementation PR, plus the destination and credential/session authorization. A generic request to continue does not authorize those mutations. Do not create, switch, or delete branches as part of this shortcut.
...
For selected ad hoc scope, disclose the complete known non-sensitive inventory in one visible English `## Scope Disclosure` section headed `Changes outside the addressed item:` and list each exact path/work unit. Do not request a reason or repeat approval per path. The disclosure does not authorize publication or separate tracking changes. Do not generate a candidate fingerprint. Use Git-native facts, existing hooks, and CI for integrity.
