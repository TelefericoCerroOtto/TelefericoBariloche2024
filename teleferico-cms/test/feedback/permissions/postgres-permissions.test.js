  ...
  try {
    ...
    await withRegisteredStrapi(port, async (strapi) => {
      ...
      try {
        ...
        const dispatchActions = registeredActions.filter((action) => action.endsWith('.dispatchFailure') || action.endsWith('.dispatchState') || action.endsWith('.workerClaim') || action.endsWith('.workerSnapshot') || action.endsWith('.workerSourceRead') || action.endsWith('.workerCheckpoint'));
        ...
        assert.deepEqual(registeredActions.filter((action) => !action.endsWith('.dispatchFailure') && !action.endsWith('.dispatchState') && !action.endsWith('.workerClaim') && !action.endsWith('.workerSnapshot') && !action.endsWith('.workerSourceRead') && !action.endsWith('.workerCheckpoint')), [
