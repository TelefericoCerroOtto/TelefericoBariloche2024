      ...
      const result = await operation({
        async lockGeneration(reportRunId) {
          ...
          const { rows } = await client.query(
            "SELECT document_id AS \"documentId\", report_run_id AS \"reportRunId\", period_start AS \"periodStart\", period_end AS \"periodEnd\", data_cutoff_at AS \"dataCutoffAt\", snapshot_digest AS \"snapshotDigest\", source_revision AS \"sourceRevision\", status, state_version AS \"stateVersion\" FROM survey_report_generations WHERE report_run_id = $1 FOR UPDATE",
            [reportRunId],
          );
          return rows[0];
        },
        ...
        async updateGeneration(patch) {
          await client.query(
            "UPDATE survey_report_generations SET status = $1, state_version = $2, completed_at = $3 WHERE report_run_id = $4",
            [patch.status, patch.stateVersion, patch.completedAt, lockedRunId],
          );
        },
      });
