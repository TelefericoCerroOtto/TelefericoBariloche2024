...
- The root readiness capability runs `npm ci`, then a `NODE_ENV=test` CMS build/start against isolated PostgreSQL and local upload storage. It remains unauthenticated, write-free, and unable to provision users, roles, permissions, or domain data; corrected teardown passed Cloud Build `7f7cc50d-4bb1-4511-8297-0d8ab3e68f39` at `b35f622eb74d24a847745eade98af6d5af594442`. The separate real-auth capability uses `scripts/provision-playwright-real-auth.js` only behind test-mode, explicit-opt-in, and exact ephemeral-database gates. It creates marker-owned synthetic `Administrator` and `Media Manager` equivalents with only exercised permissions, records bounded non-secret ownership metadata, restores the reversible service-state baseline, and removes only resources whose identity is proven. Its real Docker/Cloud Build execution remains pending; staging/production GCS, IAM, secrets, Cloud SQL, Cloud Run, staging, and production remain untouched.
