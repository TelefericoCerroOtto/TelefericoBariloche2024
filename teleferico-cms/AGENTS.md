...
- The root readiness capability runs `npm ci`, then a `NODE_ENV=test` CMS build/start against isolated PostgreSQL and local upload storage. It remains unauthenticated, write-free, and unable to provision users, roles, permissions, or domain data. The following blocking real-auth step uses `scripts/provision-playwright-real-auth.js` only behind test-mode, explicit-opt-in, and exact ephemeral-database gates. It creates marker-owned synthetic `Administrator` and `Media Manager` equivalents with only exercised permissions, records bounded non-secret ownership metadata, restores the reversible service-state baseline, and removes only resources whose identity is proven. The sanitized `scenarios:3/3` marker is withheld until synthetic verification and restoration, inner service cleanup, outer final readiness, container cleanup and absence checks, diagnostic cleanup, and selected exit `0` all succeed. Acceptance requires that marker and terminal Cloud Build success on the exact PR SHA; this repository activation has not yet supplied that runtime proof. Staging/production GCS, IAM, secrets, Cloud SQL, Cloud Run, staging, and production remain untouched.
