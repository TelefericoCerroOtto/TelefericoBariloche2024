...
- **Cloud Build Playwright baseline:** `cloudbuild.playwright-e2e.json` fails closed unless `COMMIT_SHA^{commit}` equals `/workspace` `HEAD^{commit}` and accepts only `smoke` (safe default) or `full`. Trusted same-repository implementation PRs to `development` derive smoke; the internal `development -> staging` route derives full and unrelated staging heads skip before OIDC. PR #270 proved manual same-SHA full-suite parity at `317483acb566fbeb6077a0c10a8c70f721e1c2ca` through GitHub run `34399083297` and Cloud Build `b2639432-a76e-4c65-b2f3-5c6fd779afc6`. The dispatcher workflow is now on default `main`, and the live WIF condition now permits the trusted staging route, so automatic staging OIDC full dispatch is configured but remains unproven until a later genuine eligible PR exercises it. Preserve the GitHub-hosted `chromium-smoke`, temporary `chromium-full`, and `production-public-smoke` jobs during parity, plus keyless OIDC, pinned images, `pnpm@10.33.0`, frozen installs, synthetic fixtures, and the disabled legacy trigger. Production-smoke migration and final redundant-job removal remain pending under #261. Do not treat this evidence as authenticated E2E, diagnostic artifacts, Vitest migration, deployment, trigger deletion, or authorization for further GCP/IAM changes.
