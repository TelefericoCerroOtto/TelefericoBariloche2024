...
- **Cloud Build Playwright baseline:** `cloudbuild.playwright-e2e.json` fails closed unless `COMMIT_SHA^{commit}` equals `/workspace` `HEAD^{commit}` and runs five isolated fixture/readiness steps. PR #268 proved the automatic GitHub OIDC dispatcher: run `34373237590` passed validation and dispatch, and Cloud Build `b29e46c1-10a4-4d28-bf2b-21b7d985b22d` passed all five steps with `COMMIT_SHA` and `REVISION_ID` equal to `8a0185fa70ee0dedc53573f0f6dafbffd9a4199c`; the same-SHA GitHub Chromium smoke also passed. Final legacy `/gcbrun` build `3ce4566d-030c-4dcf-b533-f9c97dc71b7c` proved parity before trigger `playwright-e2e-pr` was disabled. Future PR updates use the automatic dispatcher plus GitHub parity smoke; retain the disabled legacy trigger only for reversible rollback and delete it after a later dispatcher-only SHA proves its check does not return. The manual exact-SHA dispatcher trigger remains the fallback executor. This evidence covers only development fixture smoke, not staging, production-smoke migration, authenticated E2E, diagnostic artifacts, or final removal of redundant GitHub jobs. The dispatcher path filters still exclude their own workflow and static test; track that gap under issue #261 without changing the workflow in passive documentation work.
