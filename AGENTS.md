...
- `/implementation-pr --allow-mixed-scope "<reason>"` is the only maintainer mixed-scope override. Empty invocation remains strict; blank or unknown arguments fail closed. The agent binds the override to the exact candidate snapshot, selected `origin`, typed base/target plan, destination, and current authenticated Git/GitHub session, then passes the exact reason and exceptional path inventory to `branch-pr`. The agent-generated implementation PR must visibly disclose `## Scope Exception`; after creation, `implementation-pr` must read back and compare that disclosure before governance observation. Repository CI cannot infer whether the agent override was active. Sensitive paths, ambiguity, truncation, candidate changes, binding changes, force pushes, branch changes, rebases, merges, issue closure, branch deletion, promotions, releases, arbitrary bases, and destructive actions remain forbidden.
