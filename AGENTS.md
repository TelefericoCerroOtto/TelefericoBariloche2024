...
- Run `.github/scripts/implementation-candidate-paths.js` once against the complete candidate path list; it checks names only and never opens candidate files. Stop for unknown or secret-like paths; never read credential files or secret values. Also stop for the wrong branch, remote, or base; an existing open PR; failed or unknown commit/push; or a published head/base mismatch. Preserve one exact mixed-scope disclosure in the PR body for the complete non-sensitive inventory covered by the user's explicit reason and approval; do not ask path-by-path. Use `branch-pr`'s PR policy/content and creation clauses only, without its standalone preflight.
...
When an agent is instructed to create or update a Pull Request, it MUST NOT consider the task finished just by opening it. Because this repository enforces strict semantic checks via GitHub Actions (`backlog-governance.yml`), the agent MUST observe governance with the repository helper. For `/implementation-pr`, the first failed, timed-out, or errored observation is terminal for that invocation; diagnose the result but do not repair or retry within it. Any later metadata-only repair and observation requires fresh explicit authorization and permission under the governing contracts.
...
3. If a governance check fails—especially `Governance tests`, `validate-pr-policy`, or `trusted-pr-sync`—the agent MUST fetch the relevant failed run logs using repository-scoped GitHub CLI reads (for example, `gh run view --repo <owner>/<repo> ...`). Do not expose raw authentication status output or credential values.
4. Analyze the logs against `docs/CONVENTIONS.md` to identify the exact semantic violation (e.g., missing issue linkage, wrong title format, wrong PR type). The repository helper remains the sole governance polling/classification observer; never substitute unfiltered `gh pr checks --watch`.
5. Do not autonomously repair metadata or repeat the governance observation in the same `/implementation-pr` invocation. In another workflow, apply a metadata repair only when its current authorization permits it, then use the repository helper again. Code failures require a new implementation/finalization invocation.
