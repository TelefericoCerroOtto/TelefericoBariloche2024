...
- `/implementation-pr --allow-mixed-scope "<reason>"` is the only maintainer mixed-scope override. Empty invocation remains strict; blank or unknown arguments fail closed. The override is bound to the exact candidate snapshot, selected `origin`, typed base plan, destination, and current authenticated Git/GitHub session, and permits only non-sensitive paths classified as otherwise unrelated after exact inventory capture. The implementation PR must visibly disclose `## Scope Exception`, the reason, and exact exceptional paths/work units. Sensitive paths, ambiguity, truncation, candidate changes, binding changes, force pushes, branch changes, rebases, merges, issue closure, branch deletion, promotions, releases, arbitrary bases, and destructive actions remain forbidden.
