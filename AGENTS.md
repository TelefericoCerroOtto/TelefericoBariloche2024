...
- **Cloud Build Playwright baseline:** `cloudbuild.playwright-e2e.json` preserves the fixture-backed Chromium smoke suite and fails closed unless the trigger `COMMIT_SHA` resolves to a commit equal to `/workspace` `HEAD^{commit}`. Every Node step enables Corepack before package work because Cloud Build containers do not retain shims between steps. Its fourth fixture pilot passed for `498004d7f065e6b0a43bff42dd95c672efc2b707`; the first appended real PostgreSQL/Strapi/Next readiness pilot (`93242141-7e58-4c2d-b6f6-7673cb5eb33b`) failed when production mode loaded the GCS upload provider without a bucket. The corrected isolated runner uses `NODE_ENV=test` and Strapi's default local upload storage; staging and production GCS configuration remains untouched. Rerun `0e61da83-7490-4208-b242-ee04a95ba9b5` started PostgreSQL and reached Strapi `/admin/init` successfully, but Next.js failed before readiness because the doubled delimiter in `pnpm run dev -- --hostname 0.0.0.0 --port 3000` made `--hostname` a positional project directory. Build `323d2e61-5f11-45c3-8947-240ee3023d6a` confirmed that PostgreSQL and Strapi still succeeded and the corrected Next CLI started, then failed config validation because the local readiness runner omitted `BUILD_STRAPI_BUCKET_HOSTNAME`. The runner now also supplies the synthetic local image values `BUILD_STRAPI_BUCKET_HOSTNAME=127.0.0.1` and `BUILD_STRAPI_BUCKET_PATHNAME=/uploads/**`; a rerun is pending, so the full real stack is not yet proven. It must remain unauthenticated, write-free, synthetic, and isolated on the `cloudbuild` Docker network. Preserve GitHub Actions parity and do not add an artifact bucket, secret/IAM change, Cloud SQL, Cloud Run, staging, or production dependency without an explicitly approved work unit.
