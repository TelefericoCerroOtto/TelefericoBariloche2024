...
- **Cloud Build Playwright baseline:** `cloudbuild.playwright-e2e.json` fails closed unless `COMMIT_SHA^{commit}` equals `/workspace` `HEAD^{commit}` and accepts only `smoke` (safe default) or `full`. Trusted same-repository implementation PRs to `development` derive smoke; only internal `development -> staging` promotions derive full, while unrelated staging heads skip before OIDC. The retained GitHub parity jobs execute the same exact PR head SHA. PR #268 and dispatcher-only run `34381586085` prove the development smoke route, not the newly implemented full route. Full-suite proof requires a later promotion after an approved operator expands the WIF provider condition to allow staging base only with development head. Preserve keyless OIDC, pinned images, `pnpm@10.33.0`, frozen installs, synthetic fixtures, and the disabled legacy trigger. Do not treat this as evidence for production smoke, authenticated E2E, diagnostic artifacts, redundant-job removal, or GCP/IAM changes.
