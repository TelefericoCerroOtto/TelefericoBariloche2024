...
- The live trigger targets pull requests with base branch `development` and uses `COMMENTS_ENABLED`, so an owner or collaborator must comment `/gcbrun`. The first manually gated pilot for commit `30ef0f3` failed at smoke execution under Node 22.16.0. The second pilot (`aab57dcf-75ff-498a-89db-27c2a520e7af`) passed revision, package-manager, frozen-install, Chromium, and Playwright config loading, then failed because its final container lacked the Corepack `pnpm` shim. The third pilot (`f5b825c6-0ed6-436f-90d5-2101151d9787`) enabled Corepack in every Node step, but Chromium launch failed because `libglib-2.0.so.0` was installed in a separate build-step container. The fourth fixture pilot passed for `498004d7f065e6b0a43bff42dd95c672efc2b707` (`a4b071a2-2e78-428c-9d6d-854487f888da`, check `101890859299`). The first readiness-only pilot (`93242141-7e58-4c2d-b6f6-7673cb5eb33b`) failed before readiness when production mode loaded the GCS upload provider without `GCS_BUCKET_NAME`. The corrected runner now selects `NODE_ENV=test` before the CMS build and start, intentionally using default local upload storage; staging and production GCS settings remain untouched. Rerun `0e61da83-7490-4208-b242-ee04a95ba9b5` then started PostgreSQL and built and started Strapi, with `/admin/init` returning 200, but Next.js failed before readiness because `pnpm run dev -- --hostname 0.0.0.0 --port 3000` passed a doubled delimiter and Next interpreted `--hostname` as a positional project directory. Build `323d2e61-5f11-45c3-8947-240ee3023d6a` confirmed PostgreSQL and Strapi readiness and started the corrected Next command, then config validation failed because the runner omitted `BUILD_STRAPI_BUCKET_HOSTNAME` for `images.remotePatterns[0]`. The runner now supplies `BUILD_STRAPI_BUCKET_HOSTNAME=127.0.0.1` and `BUILD_STRAPI_BUCKET_PATHNAME=/uploads/**` as synthetic local inputs; a rerun is pending, so the full real stack is not yet proven. The digest-pinned supported Docker builder still creates only build-ID-scoped PostgreSQL and Node containers on `cloudbuild` with synthetic in-build values. No users, roles, permissions, protected writes, artifact bucket, application secret access, IAM change, Cloud SQL, Cloud Run, staging, or production integration is introduced.
