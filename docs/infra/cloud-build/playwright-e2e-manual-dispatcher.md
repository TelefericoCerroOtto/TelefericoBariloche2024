...
**Status: development smoke dispatch and manual same-SHA staging full-suite parity are proven. The dispatcher workflow and expanded WIF condition are active, so automatic staging full-suite dispatch is configured but remains unproven until a later genuine eligible PR exercises it. The manual exact-SHA trigger remains the fallback.** This snapshot records external state but does not create, update, disable, or enable it.
...
- The workflow and WIF prerequisites for automatic staging dispatch are configured. This condition preserves the repository ID, owner ID, event, trusted default-branch workflow ref, principal binding, and keyless token flow while allowing only the exact internal staging route. Automatic staging full-suite dispatch still needs proof from a later genuine eligible PR. This repository follow-up does not alter GCP or IAM.
...
- Later eligible PR updates use the configured automatic GitHub OIDC dispatcher plus the matching GitHub parity suite. The first genuine eligible `development -> staging` PR must provide automatic-dispatch proof. GitHub-hosted `chromium-smoke`, temporary `chromium-full`, and `production-public-smoke` remain during parity; `chromium-full` checks out the exact internal promotion head SHA.
...
- Automatic staging OIDC full-dispatch proof and production-smoke execution migration.
- Authenticated E2E and diagnostic artifacts.
- Final removal of redundant GitHub jobs and the disabled native trigger after parity is accepted.
