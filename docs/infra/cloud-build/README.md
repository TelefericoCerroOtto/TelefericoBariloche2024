...
The `pull_request_target` dispatcher implementation filters repository and CI-contract paths, rejects fork heads before OIDC, and derives the suite from trusted refs: `smoke` for internal PRs to `development`, and `full` only for same-repository `development` to `staging` promotions. It runs the exact head SHA through regional manual trigger `playwright-e2e-dispatch` (`3946c022-59cf-42cc-97cf-827c19f73291`) and waits for terminal status. Unrelated staging heads skip privileged execution. The dispatcher never checks out or executes PR code in GitHub. The staging route becomes active only after this workflow reaches default `main`, because `pull_request_target` uses default-branch workflow code.

PR #268 proved OIDC/WIF and exact-SHA smoke dispatch without static credentials: GitHub run `34373237590` and Cloud Build `b29e46c1-10a4-4d28-bf2b-21b7d985b22d` passed for `8a0185fa70ee0dedc53573f0f6dafbffd9a4199c`, alongside GitHub Chromium parity. Final legacy `/gcbrun` build `3ce4566d-030c-4dcf-b533-f9c97dc71b7c` passed the same five steps and SHA before `playwright-e2e-pr` was disabled. Dispatcher-only run `34381586085` and Cloud Build `bf5ce7c0-74d6-4e89-9c5e-28aadb38d645` later passed at `1dbf18ed28975fc45beb786816e42c690d304cda`. PR #269 checks prove this development smoke route only; no automatic staging full-suite evidence exists. The manual exact-SHA trigger remains the fallback executor. Keep the legacy native trigger disabled and retained. Production-smoke migration, authenticated E2E, diagnostic artifacts, redundant-job removal, Vitest migration, deployments, and legacy-trigger deletion remain out of scope under issue #261.

Bootstrap the route by merging PR #269 to `development` after review. For the first `development` to `staging` promotion, require GitHub `chromium-full` on the exact promotion head SHA and obtain explicit operational approval before manually running `playwright-e2e-dispatch` on that SHA with `_PLAYWRIGHT_SUITE=full`. This establishes initial same-SHA full-suite parity but does not prove automatic GitHub OIDC staging dispatch. Promote through `staging` and then `main` under ordinary policy. Only after the workflow reaches default `main` and an approved operator separately expands the Workload Identity Provider condition from development-only to `assertion.base_ref == 'development' || (assertion.base_ref == 'staging' && assertion.head_ref == 'development')`, while preserving all existing repository, owner, event, and trusted workflow restrictions, can a later promotion prove automatic dispatch. This repository change does not perform that GCP/IAM update.
