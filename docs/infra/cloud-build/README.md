...
The active `pull_request_target` dispatcher filters repository paths, rejects fork heads before OIDC, runs the exact head SHA through regional manual trigger `playwright-e2e-dispatch` (`3946c022-59cf-42cc-97cf-827c19f73291`), and waits for terminal status. It never checks out or executes PR code in GitHub.

PR #268 proved OIDC/WIF and exact-SHA dispatch without static credentials: GitHub run `34373237590` and Cloud Build `b29e46c1-10a4-4d28-bf2b-21b7d985b22d` passed for `8a0185fa70ee0dedc53573f0f6dafbffd9a4199c`, alongside GitHub Chromium parity. Final legacy `/gcbrun` build `3ce4566d-030c-4dcf-b533-f9c97dc71b7c` passed the same five steps and SHA before `playwright-e2e-pr` was disabled. Future PR updates use the dispatcher plus GitHub parity smoke. The manual exact-SHA trigger remains the fallback executor; retain the disabled native trigger only until a later dispatcher-only SHA proves its check does not return. Full staging, production-smoke migration, authenticated E2E, diagnostic artifacts, redundant-job removal, and dispatcher self-path coverage remain pending under issue #261.
