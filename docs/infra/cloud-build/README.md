...
The `pull_request_target` dispatcher on default `main` filters repository and CI-contract paths, including readiness and real-auth lifecycle and entrypoint files, rejects fork heads before OIDC, and derives the suite from trusted refs: `smoke` for internal PRs to `development`, and `full` only for same-repository `development` to `staging` promotions. It runs the exact head SHA through regional manual trigger `playwright-e2e-dispatch` (`3946c022-59cf-42cc-97cf-827c19f73291`) and polls for at most 4200 seconds inside a 75-minute job. Unrelated staging heads skip privileged execution. The dispatcher never checks out or executes PR code in GitHub.

Every accepted legacy profile now runs fixture coverage, independent real-stack readiness, and unconditional blocking real-auth acceptance. Acceptance requires terminal success on the exact PR SHA plus `TB122 real-auth acceptance=scenarios:3/3`. The inner lifecycle produces a bounded signal only after the structured three-scenario Playwright report, synthetic verification and restoration, and service cleanup succeed; the outer finalizer emits the public marker only after final PostgreSQL readiness, owned-container removal and absence checks, diagnostic cleanup, and selected exit `0`. This repository activation has not yet produced runtime acceptance evidence and does not authorize deployment, GCP/IAM changes, persistent resources, or production access.
...
The live WIF condition permits the exact staging route while preserving repository, owner, event, and trusted default-branch workflow restrictions. PR #273 proved automatic staging OIDC full dispatch for exact head SHA `16fab46f3781088ea4b310d6bb4e265c170372a9`: GitHub dispatcher run `34409554758` and Cloud Build `dcf5a6e9-b9b1-430d-a41e-cf98a9e84ade` succeeded with matching source revision and `COMMIT_SHA`, `_PLAYWRIGHT_SUITE=full`, and parallel GitHub full-suite and governance checks passing before merge commit `983c933f9b0a77ccdbb8472d0bcecd3bc425eb8e`. After maintainer acceptance, the redundant GitHub-hosted fixture jobs were removed; `production-public-smoke` remains unchanged. Deterministic teardown and bounded Cloud Logging diagnostics are repository-defined. Exact-SHA real-auth 3/3 runtime evidence, production-smoke migration, Vitest migration, deployments, and legacy-trigger deletion remain pending or out of scope under issue #261. Historical readiness evidence predates the activated gate and cannot satisfy it. This cutover performs no deployment or GCP/IAM change and does not complete TB-122.
