...
- The executor uses the Node 22.23.2 Bookworm image pinned by immutable digest, Corepack with the repository's `pnpm@10.33.0`, `pnpm install --frozen-lockfile`, and the lockfile-backed Playwright CLI to install only Chromium with its required system dependencies.
...
- The live `playwright-e2e-pr` Cloud Build trigger targets pull requests with base branch `development`. It uses `COMMENTS_ENABLED`, so an owner or collaborator must comment `/gcbrun` before execution. The first manually gated pilot for commit `30ef0f3` passed revision, package-manager, frozen-install, and Chromium setup steps, then failed at smoke execution under Node 22.16.0. The candidate now pins Node 22.23.2; its corrected rerun is pending, so success and check parity remain unproven. Source-upload builds without Git metadata intentionally fail closed. Failure-artifact storage still requires a separately approved bucket and access policy; this baseline has no application secrets or IAM redesign.
...
- The initial Cloud Build pilot failed before browser tests on Node 22.16.0 because Playwright 1.61.0 encountered `context.conditions?.includes is not a function` during TypeScript config loading. The baseline now pins Node 22.23.2, where the upstream 22.17+ fix applies; the rerun result remains unproven.
