...
- The dispatcher workflow is now on default `main`, and the live WIF condition permits only same-repository `development` to `staging` promotion pull requests to dispatch the exact PR head SHA with the server-derived `full` suite. Unrelated heads targeting `staging` skip the privileged dispatcher. The retained GitHub `chromium-full` parity job checks out that same PR head SHA with persisted credentials disabled and runs the complete local Chromium suite, including maintenance mode. Automatic staging OIDC full dispatch remains unproven until a later genuine eligible PR exercises it.
...
- No staging smoke job is triggered by deployment status. Promotion PRs remain covered by GitHub `chromium-full`; Cloud Build has manual same-SHA full-suite parity evidence, while the configured automatic staging path still needs real-PR proof.
...
- Manual same-SHA bootstrap is complete. PR #269 merged into `development`; PR #270 used head SHA `317483acb566fbeb6077a0c10a8c70f721e1c2ca`, where GitHub full run `34399083297` and manual Cloud Build full build `b2639432-a76e-4c65-b2f3-5c6fd779afc6` passed. PR #270 then merged into `staging` as `d46a448d996d24a421b4d4a320cb6f3e84f58506`, and PR #271 merged into `main` as `465f1f02262a67c14a2a4c8fe0c0ed16fcdbbdd2`. This proves manual full-suite parity on one immutable revision, not automatic staging dispatch.
- The live WIF provider condition now preserves repository ID `857375731`, owner ID `181292897`, event `pull_request_target`, and the trusted default-branch dispatcher workflow while allowing base `development` or the exact `development -> staging` route. The workflow and WIF prerequisites are therefore configured. A later genuine eligible `development -> staging` PR must still prove automatic GitHub OIDC full-suite dispatch.
- GitHub-hosted `chromium-smoke`, temporary `chromium-full`, and `production-public-smoke` remain during parity. Production-smoke migration, authenticated E2E, diagnostic artifacts, final redundant GitHub job removal, Vitest migration, deployments, and deletion of the disabled legacy trigger remain pending or out of scope under issue #261.
