...
The implementation workflow remains strict unless the caller uses the exact `/implementation-pr --allow-mixed-scope "<reason>"` syntax. The reason must be non-empty and the only override argument; missing, blank, unknown, or extra arguments fail closed.
...
An active override requires one visible English `## Scope Exception` section in the implementation PR body containing the non-empty reason and every exceptional entry as `Path: <exact path> | Work unit: <non-empty description>`. Same-invocation clarification may resume only when the binding remains unchanged and no terminal mutation or failure ended the invocation. Generic later follow-ups are not authorizing.
