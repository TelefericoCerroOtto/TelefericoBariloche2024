...
One explicit approval covers the complete known non-sensitive unrelated-path inventory; do not ask for separate approval per path. Obtain that approval only after the inventory is known. Preserve the exact user-approved reason and every exceptional path in one visible English `## Scope Exception` PR section. The approval does not authorize sensitive or unknown paths, an incorrect branch/remote/base, an existing open PR, ambiguous commit/push outcomes, or a published head/base mismatch. Credential files must never be read to classify paths.
...
An active override requires one visible English `## Scope Exception` section containing the exact non-empty reason and every approved entry as `Path: <exact path> | Work unit: <non-empty description>`. Verify the created PR's head and base against the published commit and authorized plan. Strict invocations omit the section. A later invocation requires fresh authorization; generic follow-ups do not authorize publication.
