...
The implementation workflow remains strict by default. The exact `/implementation-pr --allow-mixed-scope "<reason>"` syntax remains valid; an equally explicit natural-language authorization may also approve inclusion of unrelated, non-sensitive work, but must name the implementation workflow, its mutation scope, destination, and current credential/session authorization. The user must provide a concrete explanation from which the agent may draft a non-blank, one-line English audit reason. Missing/blank CLI reasons, unknown or extra arguments, ambiguous authorization, and invented rationale fail closed. This is an agent-owned publication contract; repository CI cannot infer whether the override was active.

The override is candidate-scoped and binds the complete exact snapshot, selected `origin`, typed base plan, destination, and current authenticated Git/GitHub session authorization. One explicit authorization covers the complete exact sorted non-sensitive unrelated-path inventory; do not ask for separate approval per path. Obtain that approval only after the inventory is known. It does not authorize sensitive paths, ambiguous or truncated findings, candidate or binding changes, arbitrary bases, promotions, releases, or destructive operations. The real `unrelated_count` remains in `delivery-state-snapshot.v2`; only `UNRELATED_CANDIDATE_PATHS` may be suppressed as a blocker for the unchanged valid binding. A pre-mutation blocked continuation may resolve only its identified blocker while every binding remains unchanged. Expired authorization, changed candidate/binding, or a terminal publication outcome requires fresh classification and authorization.

The ordinary publication path uses one complete bounded mapper snapshot and one explicitly authorized fetch, reusing observations and the already identified Git/GitHub session without authentication probing. Resolve the change-local route first; a direct route does not trigger generic OpenSpec/SDD discovery. If the mapper returns an inconsistent inventory, discard the whole snapshot; a count-only correction is not valid. At most one bounded complete replacement snapshot is allowed. Any changed candidate or authorization binding requires fresh explicit authorization. When the unrelated inventory is not known before capture, the mapper returns the complete exact sorted non-sensitive path list while keeping the blocker active; one approval can then bind to that list without a second discovery. Use only the compatible PR-creation parts of `branch-pr`; do not duplicate its independent discovery, branch setup, or authorization gates.
