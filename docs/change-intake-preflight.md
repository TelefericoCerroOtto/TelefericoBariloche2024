...
### Ad hoc implementation scope disclosure

The exact `/implementation-pr --allow-mixed-scope` form or an explicit natural-language scope selection may include known non-sensitive paths outside the addressed item. No reason or separate path-by-path approval is required. Empty invocation remains strict; unknown or extra CLI arguments and ambiguous scope selection fail closed. This scope selection does not authorize commit, push, PR creation, or tracking changes; each publication mutation still requires explicit authorization, destination, and current credential/session authorization. It is not a branch, base, or promotion override.

After capturing the complete exact candidate inventory, disclose each ad hoc path in one visible English `## Scope Disclosure` section as `Path: <exact path> | Work unit: <non-empty description>`, preceded by `Changes outside the addressed item:`. Verify that exact inventory in the created PR before governance observation; repository CI cannot infer whether the disclosure was active. Sensitive or unknown paths, ambiguity, truncation, candidate changes, binding changes, force pushes, rebases, merges, issue closure, branch deletion, promotions, releases, arbitrary bases, and destructive operations remain forbidden. Strict invocations omit the section.
