...
For the local code deliverable, run focused behavior and security/privacy/fail-closed tests with an injected fake provider, plus synthetic app–CMS–worker integration and isolated CMS PostgreSQL cleanup where applicable. Broad synthetic/local acceptance follows integration on `development`, in addition to required existing PR CI. Package-wide suites, typecheck/lint, and E2E remain evidence only when actually executed; Appendix-07 RED cases remain requirements. The harness uses fixed argument arrays, `finally` cleanup, and explicit volume removal; never arbitrary paths/remote databases. Do not add a Google-live Cloud Build E2E job or CI credential, and do not alter the mandatory exact-SHA Playwright fixture, real-stack readiness, real-auth acceptance, or governance CI.
...
S01 and its approved reconciliation record are live-operation gates, not prerequisites for offline implementation or fake-provider tests. Before real provider calls or generation enablement, separately approved evidence must confirm: operational, Vertex consumer/quota, billing, and telemetry project are `teleferico-bariloche-2024`; required APIs are enabled; Cloud Tasks supports `southamerica-east1`; the exact approved `gemini-3.8-flash` is used for sanitized comments at Vertex location `us` through `aiplatform.us.rep.googleapis.com`; model/deployment project, location, and endpoint match explicitly; the dedicated user-managed worker identity is attached; the distinct OIDC invoker has only invoke duty; deployment contains no service-account JSON key or `GOOGLE_APPLICATION_CREDENTIALS`. Any failed or missing gate blocks real calls, dispatch, storage/publication, and enablement; it does not block local code work. No local OpenCode identity, alternate model/location/hostname, or implicit fallback is permitted.

Order: CMS schema/index/capabilities→fixture proof→core/app readers→disabled local worker path with fake provider and synthetic app–CMS–worker integration→existing PR CI and integrated development acceptance→separately approved live Google configuration and operational checks→bounded staging smoke→generation enablement only after all gates pass→production promotion through the governed PR route. Staging proves exact revisions/config, project/location checks, keyless identities, quota/cost attribution, and synthetic flow before production promotion. Rollback pauses generation/dispatch first; intake may remain; preserve submissions/runs/reports/objects/readers; repoint version; restore compatible revisions; retain the worker identity while any revision needs it, then remove only unused TB-113 bindings after consumers stop. No deployment/deletion/operational mutation is authorized by this plan.
