...
### `TB-113 visitor form integrity correction`

- **Identity and scope:** Implemented locally on `fix/app-root-tb-113-visitor-form-integrity` from exact parent #381 head `2c3639bad542e2a893e2e433a2dc686077160668`. Corrected only visitor-form submission, success receipt, draft locale hydration, and first-invalid-control focus behavior. No branch switch, commit, push, PR, remote/GitHub/Notion/GCP operation, credential or `.env` access, dependency/install/lockfile change, deployment change, or SDD operation occurred.
- **Requirements/design references:** `specs/anonymous-feedback-intake/spec.md` requirements for QR-bound draft recovery, semantic ES/EN/PT states, authoritative receipt, and first-invalid focus; `design/02-http-contracts.md` public submit, draft, and success contracts; `design/06-migration-testing-rollout.md` RED-first app test boundary. Task 3.1 is already complete; this is a regression correction, not a formal task completion claim.
- **Changed paths and reasons:**
  - `teleferico-app/src/app/qr/feedback/[publicCode]/FeedbackForm.tsx` — submit the existing hidden honeypot's DOM value, set locale only from a valid draft resolved by the exact point/version/browser-context key, and render the authoritative response receipt in the existing success section.
  - `teleferico-app/src/app/qr/feedback/[publicCode]/FeedbackForm.test.tsx` — prove honeypot forwarding, authoritative receipt rendering, valid draft locale restoration, and stale/cross-point/cross-browser-context isolation.
  - `teleferico-app/src/lib/feedback/ui-contract.ts` and `.test.ts` — focus the first available aspect from the current catalog rather than a hard-coded key.
  - `teleferico-app/src/app/api/feedback/feedback-routes.test.ts` — assert a populated honeypot is rejected before CAPTCHA and CMS resolution.
  - `openspec/changes/tb-113-visitor-feedback/direct-implementation-ledger.md` — record this correction and its evidence.
- **Visual/OpenDesign boundary:** No layout, hierarchy, responsive presentation, or visual design change was made. Only the existing success surface gained its normative receipt value/label. `feedback-form.html` was not available locally and was not inspected; no OpenDesign-dependent design decision was made or inferred.
- **RED evidence:** `pnpm --dir teleferico-app exec vitest run 'src/app/qr/feedback/[publicCode]/FeedbackForm.test.tsx' src/app/api/feedback/feedback-routes.test.ts` — exit 1 before implementation; 3 component tests failed as expected (receipt absent, populated honeypot serialized as empty, saved locale not restored); route tests passed. `pnpm --dir teleferico-app exec vitest run src/lib/feedback/ui-contract.test.ts` — exit 1 before implementation; dynamic-catalog focus test expected `aspect-cable-car` but observed hard-coded `aspect-views`.
- **GREEN focused verification:**
  - `pnpm --dir teleferico-app exec vitest run 'src/app/qr/feedback/[publicCode]/FeedbackForm.test.tsx' src/app/api/feedback/feedback-routes.test.ts` — passed; exit 0, 2 files and 29 tests.
  - `pnpm --dir teleferico-app exec vitest run src/lib/feedback/ui-contract.test.ts src/lib/feedback/draft.test.ts` — passed; exit 0, 2 files and 7 tests, including receipt-label resolution in ES/EN/PT.
  - `pnpm --dir teleferico-app run typecheck` — passed; exit 0, no TypeScript diagnostics.
  - `git diff --check` — passed; exit 0, no whitespace errors.
- **Security and locale boundary:** Honeypot, origin/CSRF, CAPTCHA, capability, rate-limit, receipt validation, and server guard ordering were not weakened or bypassed. Receipt values are displayed only from the validated AcceptedV1 response and are not logged. A valid point/version/browser-context draft restores its saved locale; expired, cross-point, and cross-browser-context drafts do not. Normal selected-locale-to-Spanish fallback remains in the existing resolver.
- **Intentionally deferred validation:** `pnpm --dir teleferico-app exec playwright test tests/e2e/visitor-feedback.spec.ts` and browser visual/responsive acceptance — `not run`; user limited verification to focused Vitest and typecheck, and no visual redesign was made. Intended future checkpoint: implementation PR CI/integrated visitor acceptance. Owner: TB-113 implementer and reviewer.
- **Status:** `passed` for this local behavioral correction and listed focused checks only. Revision is uncommitted at the supplied exact parent; no PR or merge evidence was created. Formal SDD task checkboxes/verification/archive remain unchanged.
- **Rollback boundary:** Revert the form, form/unit/route test changes, and this ledger entry together; this removes only these visitor-form integrity corrections and preserves all existing API security and acceptance behavior.
- **Candidate authored size:** `201` additions plus deletions against exact starting HEAD `2c3639bad542e2a893e2e433a2dc686077160668`; measured using `git diff --numstat` additions plus deletions for all six tracked changed paths. This is below the standing 6,000-line TB-113 cap.
