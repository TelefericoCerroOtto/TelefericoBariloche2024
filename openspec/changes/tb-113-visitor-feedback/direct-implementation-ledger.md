...
- **Identity and scope:** Added a pure preflight for DirectV1's independently provable schema, evidence-reference, threshold, privacy, Unicode, and prohibited-marker checks on the clean branch `feat/app-root-tb-113-checkpoint-output-validation`, based at exact open parent #350 head `25b13cd748afc091fb360fb2e83d1b70e36d4902`. It never returns a validated analysis; MapV1/ReduceV1 and checkpoint acceptance remain out of scope.
- **Candidate size authorization:** For this exact branch and parent #350 head, the user approved a candidate-specific `size:exception` up to 4,000 additions plus deletions if required. The final measured candidate is 508 additions plus deletions, above the 400-line reviewer budget but below the 800-line hard default; this exception is exercised only for this candidate and does not carry forward.
...
- **Authored size:** 508 additions plus deletions against exact parent head `25b13cd748afc091fb360fb2e83d1b70e36d4902` (37 tracked additions plus deletions plus 471 lines in the two untracked files). This exceeds the ordinary 400-line reviewer budget and is covered by the exact candidate-specific exception above; the 800-line hard default remains unchanged for later slices.
...
- **Status:** `partial` under the request's rule because the first required typecheck invocation failed before the missing test imports were corrected; the exact final rerun and all other final checks passed. RED-first evidence was not captured before implementation. No key, environment, auth/grant/schema/dependency, IAM, deployment, remote, commit, PR, or SDD operation was performed.
