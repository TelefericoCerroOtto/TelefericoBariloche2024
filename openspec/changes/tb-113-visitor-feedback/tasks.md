...
| S11 → `development` after S10 merge | U7; S10, secret approval | resolver → exact answers/intake/idempotency/guards/real verification GREEN | app Vitest; Redis/verification failures | submission route/guards; security | 400 |
| S12 → `development` after S11 merge | U7; S11 | API → invariant responsive stages, validation/focus, privacy, draft, receipt, QR E2E | app Vitest/E2E; mobile+desktop synthetic QR | UI/draft; exact visitor journey | 350 |
| S13 → `development` after S12 merge | U8; S05,S09 | no admin reads → route-scoped analytics projections GREEN | app Vitest; authenticated synthetic data | read APIs; formulas/filters reconciliation | 400 |
| S14 → `development` after S13 merge | U8; S13 | reads → independent report generation, immutable history, comments/download APIs GREEN | app Vitest; authenticated API | command/download APIs; ordering/capabilities | 350 |
| S15 → `development` after S14 merge | U8; passed S03,S14 | APIs → exact four-route/module order and responsive admin E2E | app Vitest/E2E; authenticated mobile+desktop | UI/readers; invariant admin journey | 350 |
...
- [ ] 2.2 U5 is split sequentially and remains incomplete until both slices finish. S06a (authorized now, <=400 lines) adds direct deny-baseline tests under `teleferico-cms/test/feedback/permissions/**` plus `docs/STRAPI_PERMISSIONS.md`; it creates no permission script, grant, or mutation, treats D31 names as future U8 application capabilities, and defers exact intake/administration/worker actions and grants to U7/U8/U10. S06b (later) adds `teleferico-cms/scripts/seed-surveys.js` and exact ordered 13-key+`other` ES/EN/PT seed/fixtures; D:U4 then S06a; E:CMS permission inspection then seed transaction/catalog-identity/marker-cleanup; R:D05-D08,D27-D31,D92,definition+admin+platform; B:deny tests/docs then fixtures/catalog; A:permissions; L:S06a<=400,S06b TBD,CMS/docs.
