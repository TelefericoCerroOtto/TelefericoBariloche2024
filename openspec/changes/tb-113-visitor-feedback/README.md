...
- The local executor now proves synthetic zero- and nonempty-comment direct routes with injected CountTokens, analysis, and per-run evidence-key fakes; CMS-recomputed checkpoint CAS; deterministic PDF bytes; and atomic report completion/replay. Map/reduce remains fail-closed. This does not complete U10/U11/U12, activate the capability, or prove live readiness. Structurally valid nonempty analysis is not semantically certified and does not require per-report human editorial approval. See the latest direct ledger entry for exact local evidence and deferred gates.
