...
An offline dispatch coordinator is available only with both explicitly verified task-client and CMS dispatch-state ports. It reserves the deterministic name before enqueue, records only identity-verified creation, and writes `unknown` without blind retry or compensation after an ambiguous result. Retries require proof that the request never reached the task service. The default runtime supplies neither port and remains unavailable. The current CMS contract cannot record verified absence after reservation, so no `noTaskCreated`/exhausted result is emitted; that absence-and-compensation path remains a separate operational/design gate.
...
- The app worker's injected offline PDF adapter renders the eight-section/five-chart report with pinned Chromium, ECharts SVG, an embedded pinned font, accessible tables, prohibited-content rejection, and normalized PDF timestamps for stable bytes/digests. The current local Appendix-05 POC passes all nine criteria under criterion 8's static client-root/transitive-import interpretation; this is not exact production Next.js bundle analysis. Parent spotcheck also passed the focused worker/POC suite (2 files, 32 tests), app typecheck, and `git diff --check`. The synthetic direct worker path uses a local artifact store and does not change production composition or enable GCS. U12 remains unchecked until production renderer/image integration, storage/download, and operational gates are complete; no real GCS was used. See [design/05](./design/05-pdf-renderer-poc.md#offline-worker-adapter-status) and the latest ledger entry.
- The worker now has a pure, injected HTTP handler and an opt-in Node HTTP server adapter for the normative `POST /internal/v1/report-runs:execute` command. The entrypoint requires an injected verifier that returns claims only after signed-token verification, exact immutable issuer/audience/principal policy, and valid temporal claims before reading the body or invoking CMS. Synthetic loopback coverage exercises the existing executor only; there is no automatic listener, production verifier, trusted CMS origin/token provider, total execution deadline, Google integration, or deployment wiring. Keep production operation fail-closed and U10 formally unchecked until the private Cloud Run/OIDC/Tasks, deadline, and CMS runtime configuration is separately verified.
- U12 local download mediation uses a custom-token-only CMS metadata action, an authenticated app Route Handler, and injected metadata/object readers. The synthetic path checks completed-generation ownership, exact object identity, bounded PDF size, and SHA-256 before returning bytes. No GCS adapter, token, credential, persistent grant, browser-visible metadata, or production composition is included; the production reader remains fail-closed and `FEEDBACK_CAPABILITY_ENABLED` remains false. U12's formal checkbox remains open. See [HTTP contracts](./design/02-http-contracts.md#administration) and the latest ledger entry for executed evidence.
